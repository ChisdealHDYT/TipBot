...
function doBalance(message, tipper) {
  pxc.getBalance(tipper, 1, function(err, balance) {
    if (err) {
      message.reply('Error getting Flits (FLS) balance.').then(message => message.delete(10000));
    } else {
    message.channel.send({ embed: {
    description: '**:bank::money_with_wings::moneybag:Flits (FLS) Balance!:moneybag::money_with_wings::bank:**',
    color: 1363892,
    fields: [
      {
        name: '__User__',
...
function doDeposit(message, tipper) {
  getAddress(tipper, function(err, address) {
    if (err) {
      message.reply('Error getting your Flits (FLS) deposit address.').then(message => message.delete(10000));
    } else {
    message.channel.send({ embed: {
    description: '**:bank::card_index::moneybag:Flits (FLS) Address!:moneybag::card_index::bank:**',
    color: 1363892,
    fields: [
      {
        name: '__User__',
...
function doWithdraw(message, tipper, words, helpmsg) {
  ...
  if (amount === null) {
    message.reply("I don't know how to withdraw that much Flits (FLS)...").then(message => message.delete(10000));
    return;
  }
  ...
  pxc.getBalance(tipper, 1, function(err, balance) {
    if (err) {
      ...
      if (Number(amount) + Number(paytxfee) > Number(balance)) {
        message.channel.send('Please leave atleast ' + paytxfee + ' Flits (FLS) for transaction fees!');
        return;
      }
      pxc.sendFrom(tipper, address, Number(amount), function(err, txId) {
        ...
        message.channel.send({embed:{
        description: '**:outbox_tray::money_with_wings::moneybag:Flits (FLS) Transaction Completed!:moneybag::money_with_wings::outbox_tray:**',
        color: 1363892,
        fields: [
          {
            name: '__Sender__',
...
function doTip(bot, message, tipper, words, helpmsg) {
  ...
  if (amount === null) {
    message.reply("I don't know how to tip that much Flits (FLS)...").then(message => message.delete(10000));
    return;
  }
          ...
          pxc.sendFrom(tipper, address, Number(amount), 1, null, null, function(err, txId) {
              if (err) {
                ...
                if (privacyFlag) {
                  ...
                  description: '**:money_with_wings::moneybag:Flits (FLS) Transaction Completed!:moneybag::money_with_wings:**',
                  color: 1363892,
                  fields: [
                    {
                      name: '__Sender__',
                ...
                fields: [
                  ...
                  if (
                    message.content.startsWith('!tipfls private ')
                  ) {
